...
export class MapService {
    ...
    constructor(private _http: Http, private _loaderService: LoaderService) {
        ...
        this.additionalLayers = {
            ...
            tribal: esri.featureLayer({
                url:
                    "https://gis.wim.usgs.gov/arcgis/rest/services/SIGL/SIGLMapper/MapServer/4",
                style: function () {
    ...
    //subjects
    private _filteredSiteViewSubject: BehaviorSubject<any> = <
        BehaviorSubject<any>
    >new BehaviorSubject("");
    private _filteredSiteIDsSubject: BehaviorSubject<
        Array<number>
    > = new BehaviorSubject([]); //used to let sidebar know which sites are the filtered results for styling
    private _tempSiteSubject: Subject<any> = new Subject<any>();
    private _allShowingProjectIDs: Subject<Array<number>> = new Subject<
        Array<number>
    >();
    private _allOrgSystems: BehaviorSubject<any> = <BehaviorSubject<any>>(
        new BehaviorSubject("")
    );
    private _siteClicked: BehaviorSubject<any> = <BehaviorSubject<any>>(
        new BehaviorSubject({})
    );
    private _projectNameClicked: BehaviorSubject<boolean> = <
    ...
    private findPresentProps(aFeature, filters): boolean {
        ...
        let parameterArray = aFeature.properties.parameter_type_id
            ? aFeature.properties.parameter_type_id.split(",")
            : [];
        let projDurationArray = aFeature.properties.proj_duration_id
            ? aFeature.properties.proj_duration_id.split(",")
            : [];
        let projStatusArray = aFeature.properties.proj_status_id
            ? aFeature.properties.proj_status_id.split(",")
            : [];
        let resArray = aFeature.properties.resource_type_id
            ? aFeature.properties.resource_type_id.split(",")
            : [];
        let mediaArray = aFeature.properties.media_type_id
            ? aFeature.properties.media_type_id.split(",")
            : [];
        let lakeVal = aFeature.properties.lake_type_id
            ? aFeature.properties.lake_type_id
            : 0;
        let stateVal = aFeature.properties.state_province
            ? aFeature.properties.state_province
            : "";
        let monArray = aFeature.properties.monitoring_coordination_id
            ? aFeature.properties.monitoring_coordination_id.split(",")
            : [];
        let orgSysArray = aFeature.properties.organization_system_id
            ? aFeature.properties.organization_system_id.split(",")
            : [];
        let objectiveArray = aFeature.properties.objective_id
            ? aFeature.properties.objective_id.split(",")
            : [];
        ...
        if (filters.ProjectName) {
            ...
            if (filters.ORG) {
                ...
                let orgSystemsWithThisOrg = this._allOrgSystems
                    .getValue()
                    .filter(function (orgsSys) {
                ...
                orgSystemsWithThisOrg.forEach((sys) => {
                    if (
                        orgSysArray.includes(
                            sys.organization_system_id.toString()
                        )
                    ) {
                        orgHere = true;
                    }
        ...
        let finalTruth = isPresent.filter((p) => {
            return p == false;
        }).length;
    ...
    public AddTempSites(projectID: number) {
        let theseNewTemps = this._allSiteView.features.filter((feature) => {
            return (
                feature.properties.project_id == projectID &&
                this._filteredSiteIDsSubject
                    .getValue()
                    .indexOf(feature.properties.site_id) < 0
            );
        });

        if (this.temporarySites.length > 0) {
            // first see if these are already showing
            let test = this.temporarySites.filter((ts) => {
                return ts.properties.project_id == projectID;
            });
            if (test.length == 0)
    ...
    private httpRequest(): void {
        ...
        this._http
            .get(CONFIG.ORG_SYSTEM_URL, options)
        ...
        this._http
            .get(CONFIG.SITE_URL + "/GetSiteView.geojson")
            .map((res) => <any>res.json())
            .catch((err, caught) => this.handleError(err, caught))
            .subscribe((geoj) => {

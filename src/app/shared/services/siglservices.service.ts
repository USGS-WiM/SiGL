...
export class SiglService {
    ...
    // subjects
    private _parameterSubject: Subject<Array<Iparameter>> = new Subject<
        Array<Iparameter>
    >();
    private _projDurationSubject: Subject<Array<IprojDuration>> = new Subject<
        Array<IprojDuration>
    >();
    private _projStatusSubject: Subject<Array<IprojStatus>> = new Subject<
        Array<IprojStatus>
    >();
    private _resourceSubject: Subject<Array<Iresource>> = new Subject<
        Array<Iresource>
    >();
    private _mediaSubject: Subject<Array<Imedia>> = new Subject<
        Array<Imedia>
    >();
    private _lakeSubject: Subject<Array<Ilake>> = new Subject<Array<Ilake>>();
    private _stateSubject: Subject<Array<Istate>> = new Subject<
        Array<Istate>
    >();
    private _monitorEffortSubject: Subject<Array<ImonitorEffort>> = new Subject<
        Array<ImonitorEffort>
    >();
    // on init of app (and when filters are all cleared, show this list of all projects in the sidebar Project List)
    private _allProjectsSubject: Subject<Array<Ifilteredproject>> = new Subject<
        Array<Ifilteredproject>
    >();
    private _projectSubject: Subject<Array<Iproject>> = new Subject<
        Array<Iproject>
    >();
    private _siteSubject: Subject<Array<Isite>> = new Subject<Array<Isite>>();
    private _organizationSubject: Subject<Array<Iorganization>> = new Subject<
        Array<Iorganization>
    >();
    private _objectiveSubject: Subject<Array<Iobjective>> = new Subject<
        Array<Iobjective>
    >();
    private _chosenFilterSubject: Subject<any> = new Subject<any>();
    private _filteredProjectSubject: Subject<
        Array<Ifilteredproject>
    > = new Subject<Array<Ifilteredproject>>();
    ...
    private _clearAllFilters: BehaviorSubject<boolean> = <
    ...
    private setParameters(): void {
        ...
        this._http
            .get(CONFIG.PARAMETERS_URL, options)
    ...
    private setProjDurations(): void {
        let options = new RequestOptions({ headers: CONFIG.MIN_JSON_HEADERS });
        this._http
            .get(CONFIG.PROJ_DURATIONS_URL, options)
    ...
    private setProjStatuses(): void {
        let options = new RequestOptions({ headers: CONFIG.MIN_JSON_HEADERS });
        this._http
            .get(CONFIG.PROJ_STATUS_URL, options)
    ...
    private setResources(): void {
        let options = new RequestOptions({ headers: CONFIG.MIN_JSON_HEADERS });
        this._http
            .get(CONFIG.RESOURCES_URL, options)
    ...
    private setMonitorEfforts(): void {
        let options = new RequestOptions({ headers: CONFIG.MIN_JSON_HEADERS });
        this._http
            .get(CONFIG.MONITOR_EFFORTS_URL, options)
    ...
    private setProjects(): void {
        let options = new RequestOptions({ headers: CONFIG.MIN_JSON_HEADERS });
        this._http
            .get(CONFIG.PROJECT_URL, options)
        ...
        this._http
            .get(CONFIG.FILTERED_PROJECTS_URL, options)
    ...
    private setOrganizations(): void {
        let options = new RequestOptions({ headers: CONFIG.MIN_JSON_HEADERS });
        this._http
            .get(CONFIG.ORGANIZATION_URL, options)
    ...
    private setObjectives(): void {
        let options = new RequestOptions({ headers: CONFIG.MIN_JSON_HEADERS });
        this._http
            .get(CONFIG.OBJECTIVE_URL, options)
    ...
    public setFilteredSites(filters: IchosenFilters): void {
        ...
        if (Object.keys(filters).length > 0) {
            if (filters.p_organization)
                sitesParam.set("ProjOrg", filters.p_organization.toString());
            if (filters.p_objectives)
                sitesParam.set("ProjObjs", filters.p_objectives.join(","));
            if (filters.s_parameters)
                sitesParam.set("Parameters", filters.s_parameters.join(","));
            if (filters.s_projDuration)
                sitesParam.set("Duration", filters.s_projDuration.join(","));
            if (filters.s_projStatus)
                sitesParam.set("Status", filters.s_projStatus.join(","));
            if (filters.s_resources)
                sitesParam.set("ResComp", filters.s_resources.join(","));
            if (filters.s_media)
                sitesParam.set("Media", filters.s_media.join(","));
            if (filters.s_lakes)
                sitesParam.set("Lake", filters.s_lakes.join(","));
            if (filters.s_states)
                sitesParam.set("State", filters.s_states.join(","));
            if (filters.s_monitorEffect)
                sitesParam.set(
                    "ProjMonitorCoords",
                    filters.s_monitorEffect.join(",")
                );
            //if (filters.p_project) sitesParam.set("ProjMonitorCoords", filters.s_monitorEffect.join(","));
        }
        if (this.filteredSiteSubscription)
            this.filteredSiteSubscription.unsubscribe();
        // hit the filtered projects url
        let options = new RequestOptions({
            headers: CONFIG.MIN_JSON_HEADERS,
            search: sitesParam,
        });
        this.filteredSiteSubscription = this._http
            .get(CONFIG.FILTERED_PROJECTS_URL, options)
            .map((res) => <Array<Ifilteredproject>>res.json())
            .catch((err, caught) => this.handleError(err, caught))
            .subscribe((proj) => {
                ...
                if (sitesParam.paramsMap.size > 0) {
                    ...
                    if (filters.ProjectName) {
                        // project name search
                        this.setFullProject(
                            filters.ProjectName.project_id.toString()
                        );
                    }
                }
            });
    ...
    public setFullProject(projectId: string) {
        ...
        this._http
            .get(CONFIG.FULL_PROJECT_URL, options)
    ...
    public setFullSite(siteId: string) {
        let options = new RequestOptions({ headers: CONFIG.MIN_JSON_HEADERS });
        this._http
            .get(CONFIG.SITE_URL + "/" + siteId + "/GetFullSite", options)
    ...
    private appversion: BehaviorSubject<string> = <BehaviorSubject<string>>(
        new BehaviorSubject("")
    );

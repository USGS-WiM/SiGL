...
export class SidebarComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this._mapService.siteClicked.subscribe((site) => {
            if (Object.keys(site).length > 0) {
                ...
                if (site.site_id) {
                    ...
                    this.selectedSite = !(this.siteNameClickOnOff % 2 == 0)
                        ? site.site_id
                        : 0;
        ...
        this.sortByObject = [
            ...
            {
                title: "Filtered Site Count: ",
                icon: "&#xf162;",
                sortBy: "FilteredSiteCnt",
                direction: "asc",
            },
            {
                title: "Filtered Site Count: ",
        ...
        this.filterCount = 0;
        this.siteFilters = false;
        this.projectFilters = false;
        //site toggle button group form
        this.siteCountForm = this._formBuilder.group({
            siteToggle: "filtered",
        });
        this.filteredProjects = [];
        this.allFilteredProjectsHolder = [];
        this.allProjects = [];
        this.allProjectsHolder = [];
        ...
        this._siglService.chosenFilters.subscribe((choices: IchosenFilters) => {
            ...
            if (
                choices.p_objectives ||
                choices.s_monitorEffect ||
                choices.s_projDuration ||
                choices.s_projStatus ||
                choices.ProjectName
            )
        ...
        this._siglService.allProjects.subscribe(
            (allProjects: Array<Ifilteredproject>) => {
                ...
                this.allProjects.sort((leftSide, rightside): number => {
                    if (
                        leftSide.name.toLowerCase() <
                        rightside.name.toLowerCase()
                    )
                        return -1;
                    if (
                        leftSide.name.toLowerCase() >
                ...
                this.allProjectsHolder = this.allProjects.map((x) =>
                    Object.assign({}, x)
                );
        ...
        this._siglService.filteredProjects.subscribe(
            (projects: Array<Ifilteredproject>) => {
                ...
                if (projects.length > 0) {
                    ...
                    projects.forEach((p: Ifilteredproject) => {
                        ...
                        p.projectSites.forEach((s: Isimplesite) => {
                            if (s.isDisplayed) p.filteredSiteCount++;
                        });
                ...
                this.allFilteredProjectsHolder = this.filteredProjects.map(
                    (x) => Object.assign({}, x)
                );
                //check to see if projects without sites are hidden
                if (!this.projectsWithSitesShowing) {
                    this.filteredProjects = this.allFilteredProjectsHolder.filter(
                        (proj: Ifilteredproject) => {
                            return proj.projectSites.length > 0;
                        }
                    );
                    if (this.filteredProjects.length == 0) {
                        //if all filtered projects have no sites
                        this.NoMatches = true;
                    }
                } else {
                    if (
    ...
    public showProjectDetails(project: any, fromToggle: boolean): void {
        ...
        gtag("event", "click", {
            event_category: "ProjectList",
            event_label: "ProjectNameClick: " + projID,
        });
    }
    ...
    public showSiteDetails(site: Isimplesite): void {
        ...
        gtag("event", "click", {
            event_category: "ProjectList",
            event_label: "SiteNameClick: " + site.site_id,
        });
    ...
    public toggleSiteList(value: string, projectId: number) {
        ...
        this.showProjectDetails(
            this.filteredProjects.filter((fp) => {
                return fp.project_id == projectId;
            })[0],
            true
        );
        ...
        gtag("event", "click", {
            event_category: "ProjectList",
            event_label: "ProjectId: " + projectId + ", Toggle: " + value,
        });
            ...
            if (p.project_id == projectId) {
                ...
                if (value == "all") {
                    ...
                    if (this.AllShowingProjIds.indexOf(projectId) > -1) {
                        this.AllShowingProjIds.splice(
                            this.AllShowingProjIds.indexOf(projectId),
                            1
                        );
                    }
                }
        ...
        if (fromWhere == "click")
            gtag("event", "click", {
                ...
                event_label:
                    ...
                    chosenSort.sortBy +
                    " : " +
                    chosenSort.direction,
            });

        // need to know which list of projects we are sorting (filtered or all)
        if (this.showAllProjects) {
            if (
                chosenSort.sortBy == "ProjectName" &&
                chosenSort.direction == "asc"
            ) {
                this.allProjects.sort((leftSide, rightside): number => {
                    if (
                        leftSide.name.toLowerCase() <
            ...
            if (
                chosenSort.sortBy == "TotalSiteCnt" &&
                chosenSort.direction == "asc"
            ) {
                this.allProjects.sort((leftSide, rightside): number => {
                    if (
                        leftSide.projectSites.length <
                        rightside.projectSites.length
                    )
                        return -1;
                    if (
                        leftSide.projectSites.length >
            ...
            if (
                chosenSort.sortBy == "FilteredSiteCnt" &&
                chosenSort.direction == "asc"
            ) {
                this.filteredProjects.sort((leftSide, rightside): number => {
                    if (
                        leftSide.filteredSiteCount < rightside.filteredSiteCount
                    )
                        return -1;
                    if (
                        leftSide.filteredSiteCount > rightside.filteredSiteCount
    ...
    public toggleProjectsWithSites(onOrOff) {
        if (this.projectsWithSitesShowing) {
            ...
            if (this.showAllProjects)
                this.allProjects = this.allProjectsHolder.filter(
                    (proj: Ifilteredproject) => {
                        return proj.projectSites.length > 0;
                    }
                );
            ...
            this.projectsWithSitesShowing = true;
            if (this.showAllProjects)
                this.allProjects = this.allProjectsHolder.map((x) =>
                    Object.assign({}, x)
                );
            else
                this.filteredProjects = this.allFilteredProjectsHolder.map(
                    (x) => Object.assign({}, x)
                );
            this.NoMatches = false;
        }
    ...
    public downloadCSV() {
        ...
        if (this.filteredProjects.length > 0) {
            ...
            csvDataHolder = this.filteredProjects.map((x) =>
                Object.assign({}, x)
            ); //deep copy
    ...
    private ConvertToCSV(objArray) {
        ...
        var currentDate = Date.now();
        var datePipe = new DatePipe("en-US");
        var filteredDate = datePipe.transform(
            currentDate,
        ...
        if (this.chosenFilters) {
            ...
            if (this.projectFilters) {
                if (filt.DURATIONS) {
                    ...
                    for (
                        let s = 0;
                        s < this.chosenFilters.DURATIONS.length;
                        s++
                    ) {
                ...
                if (filt.STATUSES) {
                    ...
                    for (
                        let s = 0;
                        s < this.chosenFilters.STATUSES.length;
                        s++
                    ) {
                ...
                if (filt.MONITORS) {
                    ...
                    for (
                        let s = 0;
                        s < this.chosenFilters.MONITORS.length;
                        s++
                    ) {
            ...
            if (this.siteFilters) {
                if (filt.PARAMETERS) {
                    ...
                    for (
                        let s = 0;
                        s < this.chosenFilters.PARAMETERS.length;
                        s++
                    ) {
                        str += this.chosenFilters.PARAMETERS[s].name + "; ";
                    }
                ...
                if (filt.RESOURCES) {
                    ...
                    for (
                        let s = 0;
                        s < this.chosenFilters.RESOURCES.length;
                        s++
                    ) {
        ...
        if (this.showAllProjects)
            row =
                "Project Name, Project ID, Organization(s), Full Project Information, Site ID, Site name, Latitude, Longitude, Country, State/Province, Lake";
        else
            row =
                "Project Name, Project ID, Organization(s), Full Project Information, Site ID, Site name, Filtered Result, Latitude, Longitude, Country, State/Province, Lake";
                    ...
                    if (array[i]["Organizations"] == array[i][index]) {
                        ...
                        for (let s = 0; s < array[i][index].length; s++) {
                            if (s == 0) {
                                ...
                                if (this.showAllProjects) {
                                    line +=
                                        "https://sigl.wim.usgs.gov/SiGLServices/projects/GetFullProject.json?ByProject=" +
                                        array[i][index][s].project_id +
                                        "," +
                                        array[i][index][s].site_id +
                                        "," +
                                        array[i][index][s].name.replace(
                                            /,/g,
                                            " "
                                        ) +
                                        "," +
                                        array[i][index][s].latitude +
                                        "," +
                                        array[i][index][s].longitude +
                                        "," +
                                        array[i][index][s].Country +
                                        "," +
                                        array[i][index][s].State +
                                        "," +
                                        array[i][index][s].Lake +
                                        "\r\n";
                                } else {
                    ...
                    for (let s = 0; s < array[i][index].length; s++) {
                        if (s == 0) {
                            line +=
                                ...
                                array[i][index][s].site_id +
                                "," +
                                array[i][index][s].name.replace(/,/g, " ") +
                                "," +
                                array[i][index][s].latitude +
                                "," +
                                array[i][index][s].longitude +
    ...
    public toggleLayer(newVal: string) {
        ...
        if (index > -1) {
            ...
            this._mapService.map.removeLayer(
                this._mapService.additionalLayers[newVal]
            );
        } else {
